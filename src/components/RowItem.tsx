...
const RowItem: React.FC<RowItemProps> = ({
  ...
  return (
   <>
    <div
      className={`md:flex grid text-sm md:text-md font-medium mt-[12px]  grid-cols-[40px,1fr] grid-rows-[auto,auto,auto] text-${textColor}  `}
      onClick={handleClick}
      key={key}
    >
      <p className="md:mr-[7px] row-start-1 col-start-1 ">{sno + ")"}</p>
      <p className={`md:w-[84%] row-start-1 col-start-2 text-${paraColor}`}>{text}</p>
